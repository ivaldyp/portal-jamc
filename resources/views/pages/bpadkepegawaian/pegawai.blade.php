...
@section('css')
	...
	<link rel="stylesheet" href="/{{ config('app.name') }}{{ ('/public/ample/plugins/bower_components/html5-editor/bootstrap-wysihtml5.css') }}" />
	<!-- animation CSS -->
	...
	<div id="page-wrapper">
		<div class="container-fluid">
                    	...
                    	<div class="panel-wrapper collapse in">
                            <div class="panel-body">
                            	<div class="row " style="margin-bottom: 10px">
                            		@if ($access['zadd'] == 'y')
                            		<div class="col-md-1">
				                      	<a href="/{{ config('app.name') }}/kepegawaian/tambah pegawai"><button class="btn btn-info col-md-12" type="button">Tambah</button></a>
                            		</div>
                            		@endif
                            		<div class="col-md-1">
										<a href="/{{ config('app.name') }}/kepegawaian/excelpegawai?unit={{ $idunit }}&ked={{ $kednow }}"><button class="btn btn-success col-md-12" type="button"> Excel </button></a>
									</div>
                            		<div class="col-md-10">
                            			<form method="GET" action="/{{ config('app.name') }}/kepegawaian/data pegawai">
					                      	<div class=" col-md-2">
										...
										<table class="myTable table table-hover">
											...
											<tbody style="vertical-align: middle;">
												...
												<tr <?php if(strlen($employee['idunit']) < 10): ?> style="font-weight: bold;" <?php endif ?>  >
													...
													@if($access['zupd'] == 'y' || $access['zdel'] == 'y')
														<td>
															<form method="GET" action="/{{ config('app.name') }}/kepegawaian/ubah pegawai">
															@if($access['zupd'] == 'y')
															...
															<div id="modal-password-{{$key}}" class="modal fade" role="dialog">
																<div class="modal-dialog">
																	<div class="modal-content">
																		<form method="POST" action="/{{ config('app.name') }}/kepegawaian/form/ubahpassuser" class="form-horizontal">
																		@csrf
			...
			<div id="modal-delete" class="modal fade" role="dialog">
				<div class="modal-dialog">
					<div class="modal-content">
						<form method="POST" action="/{{ config('app.name') }}/kepegawaian/form/hapuspegawai" class="form-horizontal">
						@csrf
							<div class="modal-header">
...
@section('js')
    ...
    <script src="/{{ config('app.name') }}{{ ('/public/ample/plugins/bower_components/html5-editor/wysihtml5-0.3.0.js') }}"></script>
    <script src="/{{ config('app.name') }}{{ ('/public/ample/plugins/bower_components/html5-editor/bootstrap-wysihtml5.js') }}"></script>
    <script>
	...
	<script>
		$(function () {
			...
			$('.btn-update').on('click', function () {
				...
				$("#modal_update_href").attr("href", "/{{ config('app.name') }}/cms/form/apprcontent?ids=" + ids + "&idkat=" + idkat + "&appr=" + appr + "&judul=" + judul );
